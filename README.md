...
Install Android on a connected emulator or device:

```bash
./gradlew :manual-testing:android:installDebug
```

These manual testing apps depend on the SDK source directly:

```kotlin
implementation(project(":shared"))
```

So you can edit the SDK and test locally without publishing to Maven Central.
...
```text
KitFlow is a Kotlin Multiplatform SDK for adaptive Compose UI.

The goal is simple: write adaptive values once and use them naturally with existing Compose APIs.

It helps with breakpoints, orientation-aware UI, accessibility-aware layout decisions, and stable responsive code arrangement without replacing Material or Compose components.
```
